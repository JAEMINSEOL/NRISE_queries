...
, jelly_use as(
    ...
    join (select user_group,ci_hash, user_id from users) u on ju.user_id = u.user_id
...
select j.*
        , case when j.description like '%한일%' then '한일'
                ...
                when j.description like '%프로필 평가%' or j.description like '%닮은꼴%' or j.description like '%설문%' or j.description like '%리포트%' then '리포트'
