...
, user_jelly as(
    select u.user_group, u.ci_hash,u.first_approval_date
...
select *
     ...
     , avg(daily_jelly_income) over (partition by user_group order by first_approval_date rows between unbounded preceding and current row) as cumul_jelly_income
    , avg(daily_arpu) over (partition by user_group order by first_approval_date rows between unbounded preceding and current row) as cumul_arpu
from s1
