...
select u.relative_score,j.*
        ...
        , p.description as purchased_item
...
select * from (select relative_score
                    , user_group
                    , nth_day
                    , action_num
                    , get_referral_reward
                    ...
                    , purchased_item
                    , purchase_num
                    , registered_time as purchased_time
                    , lead(registered_time) over (partition by ci_hash order by action_num) - registered_time as purchase_use_interval
        , lead(description) over (partition by ci_hash order by action_num) as next_action_description
        , lead(use_type) over (partition by ci_hash order by action_num) as next_action_type
               from summary s)
where purchase_num = 1
order by get_referral_reward, user_group, purchased_item, relative_score
