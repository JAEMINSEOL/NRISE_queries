...
select *
     , num_profile_opened_users_today *100.0 / num_today_rcmd_users as rate_profile_today
      , num_chat_request_users_today *100.0 / num_today_rcmd_users as rate_request_today
        , num_profile_opened_users_related *100.0 / num_related_rcmd_users as rate_profile_related
from (select date_ymd_kst
           , exp_var
           ...
           , count(case
                                when channel_id = 21 and not rcmd_type = 2060 and profile_open_time is not null
                                    then user_id end)                                                            as num_profile_opened_users_today
           , count(case
                                when channel_id = 21 and not rcmd_type = 2060 and friend_request_time is not null
                                    then user_id end)                                                            as num_chat_request_users_today
           , count(case
                                when rcmd_type = 2060 and profile_open_time is not null
                                    then user_id end)                                                            as num_profile_opened_users_related
      from (select r.user1_id as user_id,
            ...
            from (select *
                  ...
                  where date_ymd_kst > '2025-11-27'
                    and (rcmd_type = 2060 or channel_id = 21)) r
                     join (select *
